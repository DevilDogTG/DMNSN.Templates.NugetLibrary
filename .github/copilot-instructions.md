...
2. This workspace's project is `DevilDogTG/DMNSN.Templates.NugetLibrary` (derived from `git remote get-url origin` at
   onboarding). Re-run the onboard script if the remote changes.
3. Read the project directives from `~\.agent-brains\projects\DevilDogTG/DMNSN.Templates.NugetLibrary\AGENT.md`.
4. Use `~\.agent-brains\projects\DevilDogTG/DMNSN.Templates.NugetLibrary\memory\` for project context.
5. Always write plans to `~\.agent-brains\projects\DevilDogTG/DMNSN.Templates.NugetLibrary\plan\` BEFORE writing code.
...
<!-- SKILLS:START -->
<!-- Last updated: 2026-07-24 | Source: global + profile:base-developer + workspace -->
...
**Invoke when:** Execute this skill when the user wants a recap of the current or recent session **without** closing the session. This skill is read-only: it summarizes work, decisions, and blockers, but it does not perform handover, plan lifecycle changes, git hygiene steps, or commits. Use this skill for requests like: - "summarize this session" - "what did we do?" - "give me a recap before we continue" If the user is explicitly ending work for now, use `session-end` instead.
**Steps:**
1. Summarize the session in 3-7 bullet points:
2. Call out current state in 1-3 bullets:
3. Do not mutate workspace state:
